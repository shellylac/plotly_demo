...
p_scatter <- plot_ly(
  ...
  name = "scatter" # Naming traces can help with legends and other interactive features
) |>
  layout(
...
p_line <- plot_ly(
  ...
  line = list(color = "red"),
  name = "line" # Naming traces can help with legends and other interactive features
) |>
  layout(
    title = "Car Weight vs MPG",
    xaxis = list(title = "Weight (1000 lbs)"),
...
p_bar <- plot_ly(
  ...
  layout(
    ...
    legend = list(
      title = list(text = "Engine size (cylinders)")
    )
  )
...
```
 ...
 - **NOTE** see [Client Side Linking](https://plotly-r.com/client-side-linking) - for ways to link hover info and highlighting across multiple figures in a subplot
...
# Look at it
# utils::browseURL(here('output', 'my_plotly_figure.html'))
```
...
pyramid_death_data <- read_csv(
  here('input', 'deaths_pyramid_data.csv'),
...
```{r pyramid_plot}
# Pyramid plot for Queensland only
...
pyramid_by_geo <- plot_ly() |>
  ...
  add_bars(
    data = filter(pyramid_death_data_recoded, geo_label == "Australia"),
    x = ~prop,
    y = ~age_label,
    color = ~sex_label,
    colors = c("#563188", "#BBA4D9"),
...
mort_data <- read_csv(
  here('input', 'mort_pyramid_data.csv'),
  show_col_types = FALSE
)
...
mort_animation_plot <-
        plot_ly(data = mort_data_recoded) |>
          ...
          x = ~prop,
          y = ~age_label,
          frame = ~time_period, # Splits the data by unique year
          color = ~sex_label,
          type = 'bar',
          orientation = 'h',
          colors = c("#563188", "#BBA4D9"),
...
-   Finally - you can combine animations with the same frames - so you can produce something like this:

```{r open asr_combo_figure}
utils::browseURL(here('output', 'mort_asr_comb_animation.html'))
```

------------------------------------------------------------------------

# But What about the magic of `ggplotly()`
...
## Not COMPLETELY magical

-   poor hover customization (can't format numbers etc)
-   secondary y-axes don't work at all in ggplotly
-   annotations: shape and text annotations don't convert reliably
-   No 3D visualization support
-   Won't translate subplot type figs very well
-   Some ggplot themes will break it
-   AND many more issues!!

```{r ggplotly_hoverissues}
mtcars_formatted <- mtcars |>
  mutate(
    car_name = rownames(mtcars),
    cyl = factor(cyl)
  )

# Need to use text aesthetic to modify hover info - but still limited
p1_text <- ggplot(mtcars_formatted, aes(wt, mpg, color = cyl)) +
  geom_point(aes(text = paste0("POINT: ", car_name)), size = 2.6, alpha = 0.75) +
  geom_smooth(aes(text = paste0("SMOOTH: cyl=", cyl)), se = FALSE, linewidth = 1) +
  theme_minimal() 
p1_text
ggplotly(p1_text)
```
