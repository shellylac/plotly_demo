...
p_line <- plot_ly(data = mtcars_sorted, 
        x = ~wt, 
        y = ~mpg, 
        type = 'scatter',
...
```{r subplots}
htmlwidgets::saveWidget(widget = p_subplot, file = here('output', 'my_plotly_figure.html'))
```
...
p_scatter <- p_scatter |> 
  ...
  modeBarButtonsToRemove = c("sendDataToCloud", "toggleSpikelines", "zoom2d", "pan2d", "select2d", "lasso2d")
    ) 
p_scatter
```
